...
impl ScriptThread {
    ...
    fn handle_msg_from_constellation(&self, msg: ConstellationControlMsg) {
        match msg {
            ...
            ConstellationControlMsg::TransitionEnd(unsafe_node) =>
                self.handle_transition_event(unsafe_node),
    ...
    #[allow(unsafe_code)]
    fn handle_transition_event(&self, unsafe_node: UnsafeNode) {
        let node: &Node = unsafe { transmute(ServoLayoutNode::from_unsafe(&unsafe_node).get_jsmanaged().unsafe_get()) };
        let init = TransitionEventInit {
            parent: EventInit {
                bubbles: true,
                cancelable: false,
            },
            propertyName: DOMString::new(),
            elapsedTime: Finite::new(0.0).unwrap(),
            pseudoElement: DOMString::new()
        };
        let transition_event = TransitionEvent::new(node.global().r(),
                                                    DOMString::from("transitionend"),
                                                    &init);
